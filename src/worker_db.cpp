...
namespace ds {
        ...
        if (this != &other) {
            ...
            if (other.capacity_ > 0) {
                ...
                for (std::size_t i = 0; i < other.size_; ++i) {
                    new_entries[i] = other.entries_[i];
                }
            }
            delete[] entries_;
    ...
    bool WorkerDb::contains(const char* surname) const {
        return contains(MyString(surname));
    }

    bool WorkerDb::contains(const MyString& surname) const {
        return find_index(surname) != size_;
    }

    bool WorkerDb::erase(const char* surname) {
        return erase(MyString(surname));
    }

    bool WorkerDb::erase(const MyString& surname) {
        std::size_t index = find_index(surname);
        if (index == size_) {
            return false;
        }
    ...
    std::size_t WorkerDb::size() const {
        return size_;
    }
