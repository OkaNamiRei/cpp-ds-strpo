...
//todo чем отличаются 29 и 27 строчки
//Первый оператор — это копирующее присваивание. Он принимает const Matrix&, то есть ссылку на существующий объект, который нельзя менять, и копирует
// его данные в текущий объект.
//Второй оператор — это перемещающее присваивание.Он принимает Matrix&&, то есть rvalue - ссылку на временный объект или объект, который явно передали
// через std::move.В идеале move должен не копировать ресурс, а забирать владение ресурсом.Но в этой реализации у Matrix нет динамической памяти,
// поэтому move фактически почти не даёт выигрыша и похож на копирование.
//&& - rvalue ссылка, может привязываться к временным объектам (rvalue), позволяя продлевать им «жизнь» и изменять их, в мув семантикс забирает ресурсы
//временного обьекта в новый
