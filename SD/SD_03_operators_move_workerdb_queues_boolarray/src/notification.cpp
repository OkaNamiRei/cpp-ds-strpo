...
std::size_t count_notifications_by_type(const Notification* notifications, std::size_t count, NotificationType type) {
    std::size_t total = 0;
    for (std::size_t i = 0; i < count; ++i) if (notifications[i].type == type) ++total;
    return total;
}

NotificationQueue::Iterator::Iterator(const NotificationQueue* owner, std::size_t offset) : owner_(owner), offset_(offset) {}
...
bool NotificationQueue::Iterator::operator==(const Iterator& other) const { return owner_ == other.owner_ && offset_ == other.offset_; }
...
NotificationQueue::NotificationQueue(const NotificationQueue& other) : data_(nullptr), capacity_(0), size_(0), head_(0) {
    reserve(other.capacity_);
    size_ = other.size_;
    for (std::size_t i = 0; i < size_; ++i) data_[i] = other.at_offset(i);
}
NotificationQueue::NotificationQueue(NotificationQueue&& other) noexcept
    ...
    other.data_ = nullptr; other.capacity_ = 0; other.size_ = 0; other.head_ = 0;
}
NotificationQueue& NotificationQueue::operator=(const NotificationQueue& other) {
    if (this != &other) {
        ...
        for (std::size_t i = 0; i < other.size_; ++i) new_data[i] = other.at_offset(i);
        delete[] data_;
        data_ = new_data; capacity_ = other.capacity_; size_ = other.size_; head_ = 0;
    }
    return *this;
}
...
const Notification& NotificationQueue::at_offset(std::size_t offset) const { return data_[(head_ + offset) % capacity_]; }

void NotificationQueue::reserve(std::size_t new_capacity) {
    if (new_capacity <= capacity_) return;
    Notification* new_data = new Notification[new_capacity];
    for (std::size_t i = 0; i < size_; ++i) new_data[i] = at_offset(i);
    delete[] data_;
    data_ = new_data; capacity_ = new_capacity; head_ = 0;
}
...
Notification NotificationQueue::pop() {
    if (empty()) throw std::out_of_range("notification queue is empty");
...
NotificationPriorityQueue& NotificationPriorityQueue::operator=(const NotificationPriorityQueue& other) {
    if (this != &other) {
        ...
        delete[] data_;
        data_ = new_data; size_ = other.size_; capacity_ = other.capacity_;
    }
    return *this;
}
...
bool NotificationPriorityQueue::is_more_actual(const Notification& lhs, const Notification& rhs) {
    bool lhs_urgent = lhs.type == NotificationType::kSystem && lhs.payload.system.severity == NotificationSeverity::kUrgent;
    bool rhs_urgent = rhs.type == NotificationType::kSystem && rhs.payload.system.severity == NotificationSeverity::kUrgent;
    if (lhs_urgent != rhs_urgent) return lhs_urgent;
    if (lhs.timestamp != rhs.timestamp) return lhs.timestamp < rhs.timestamp;
    int lhs_rank = type_rank(lhs), rhs_rank = type_rank(rhs);
...
Notification NotificationPriorityQueue::pop() {
    if (empty()) throw std::out_of_range("notification priority queue is empty");
    std::size_t best = 0;
    for (std::size_t i = 1; i < size_; ++i) if (is_more_actual(data_[i], data_[best])) best = i;
    Notification result = data_[best];
    for (std::size_t i = best + 1; i < size_; ++i) data_[i - 1] = data_[i];
    --size_;
    return result;
}
